...
def Authority_Login(request):
    # conn2 = mysql.connector.connect(host='localhost', user='root', password='', database='student_management_system')
    if request.method == 'POST':
        ...
        if models.Authorregiss.objects.filter(Email=User_email, Password=User_password):
            return redirect('athurpage')
        else:
            return HttpResponse("password & username not matching!")
...
def Teacher_login(request):
    if request.method == 'POST':
        ...
        Email = request.POST.get('Email')
        Password = request.POST.get('Password')
        Mydata = models.Teacherreg.objects.filter(Teacher_Id=Teacher_Id, Email=Email, Password=Password)
        if Mydata:
            ...
            if data:
                ...
                return HttpResponse("this teacher id not fund")
        else:
            return HttpResponse("Information not matching")
...
def Teacher_reg(request):
    if request.method == 'POST':
        ...
        time = request.POST.get('time')
        Mydata = models.AddTeacherpage.objects.filter(Teacher_Id=teacher_id)
        if Mydata:
...
def Student_login(request):
    conn3 = mysql.connector.connect(host='localhost', user='root', password='', database='student_management_system')
    if request.method == 'POST':
...
def Student_reg(request):
    if request.method == 'POST':
        ...
        # data = cur.fetchone()
        # print(data, student_id)
        Mydata = models.Addstupage.objects.filter(Student_Id=student_id)
        if Mydata:
            if password == con_password:
                ...
                return HttpResponse("password & confirm password must same!")
        else:
            return HttpResponse("this student id not found")
